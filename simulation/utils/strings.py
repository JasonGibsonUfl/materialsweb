"""
simulation.data.strings

Contains useful functions for string manipulation. Some important conventions
observed throughout simulation that are particularly relevent here:
...
def electronegativity(elt):
    if not elt in data:
        ...
        if not 'electronegativity' in data[elt]:
            return 0.0
        return data[elt]['electronegativity']
...
def read_compressed_array(string):
    if '-' in string:
        ...
        vals = list(map(lambda x: -float(x), string.split('-')))
        if not first:
            vals[0] = -vals[0]
        return vals
