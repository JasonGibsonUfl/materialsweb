...
import simulation
import simulation.materials.structure as st
#from simulation.materials.structure import Structure as st
import simulation.materials.composition as comp
import simulation.data.meta_data as ref
...
def write(struct, filename=None, comments=None, direct=True,
    ...
    """
    Write a :mod:`~simulation.Structure` to a file or string.

    Arguments:
        struct: A `~simulation.Structure` object.
...
def read(poscar, species=None):
    ...
    if not vasp5:
        ...
        if not species:
            ...
            for s in _species:
                if not s in simulation.elements.keys():
