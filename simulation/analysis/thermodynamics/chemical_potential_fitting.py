...
def fit(name, calculations=None, experiments=None, fit_for=[]):
    ...
    base_mus = dict( (elt, simulation.Composition.get(elt).total_energy) for elt in
            element_groups['all'])
    ...
    for elt, mu in base_mus.items():
        if elt not in element_mus:
            element_mus[simulation.Element.get(elt)] = float(mu)

        if abs(element_mus.get(simulation.Element.get(elt), 0.0)) > 100:
            element_mus[simulation.Element.get(elt)] = float(mu)
    ...
    for elt, val in element_mus.items():
        ...
        mu = simulation.ReferenceEnergy(element_id=elt, value=val)
        f.reference_energy_set.add(mu)
        new_e_mus[str(elt)] = float(val)

    new_h_corrs = {}
    for hub, val in hubbard_mus.items():
        hm = simulation.HubbardCorrection(hubbard=hub, value=val, element=hub.element)
