# simulation/analysis/thermodynamics/phase.py
...
import logging
import materialsweb2
from simulation.utils import *
from django.db.models import F, Q
import operator
from simulation.utils.strings import format_comp, format_html, parse_comp, format_latex, reduce_comp, unit_comp
from simulation.materials.structure import Structure
logger = logging.getLogger(__name__)

THERMOPY_LIB_PATH = materialsweb2.settings.INSTALL_PATH + '/data/thermodynamic/'
...
class PhaseData(object):
    ...
    def load_library(self, library):
        ...
        self.read_file(simulation.INSTALL_PATH + '/data/thermodata/' + library)
    ...
    def load_oqmd(self, space=None, search={}, exclude={},
        ...
        """
        from simulation.materials.formation_energy import FormationEnergy
        ...
        if space:
            ...
            combined_q = Structure.reduce(operator.or_, element_q_lst)
            combined_q = Structure.reduce(operator.and_, [combined_q, Q(composition__ntypes__lt=dim)])

            exclude_element_q_lst = [Q(composition__element_list__contains=e.symbol + '_')
                                     for e in Element.objects.exclude(symbol__in=space)]
            combined_q_not = Structure.reduce(operator.or_, exclude_element_q_lst)
    ...
    def read_file(self, filename, per_atom=True):
        ...
        if isinstance(filename, str):
            ...
            fileobj = filename
        elif isinstance(filename, type(io.StringIO.StringIO())):
            fileobj = filename
            fileobj.name = None
    ...
    def get_phase_data(self, space):
        ...
        Arguments:
            space: formatted as in :func:`simulation.PhaseSpace.__init__()`
...
class Phase(object):
    ...
    def formation(self):
        ...
        if self._formation is None:
            self._formation = simulation.FormationEnergy.objects.get(id=self.id)
