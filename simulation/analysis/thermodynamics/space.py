# simulation/analysis/thermodynamics/space.py
...
class PhaseSpace(object):
    ...
    def chempot_bounds(self, composition, total=False):
        ...
        for eq in self.hull_list:
            ...
            if total:
                for k in pots:
                    pots[k] += simulation.chem_pots['standard']['elements'][k]
            chems[eq] = pots
        return chems
    ...
    def compute_stabilities(self, phases=None, save=False, reevaluate=True):
        ...
        for p in phases:
            ...
            if save:
                qs = simulation.FormationEnergy.objects.filter(id=p.id)
