...
## This function gives: "return uv_opt[0], uv_opt[1]"
    ...
    if found:
        ...
        return uv_opt[0], uv_opt[1], uv_all
    else:
        #print('\n NO MATCH FOUND\n')
        return None, None
...
def get_aligned_lattices(uv_substrate, uv_mat2d, slab_sub, slab_2d,
                         max_angle_diff=1):
    """
...
def run_lat_match(substrate, twod_layer, match_constraints, all_matches=False):
    ...
    uv_substrate, uv_mat2d, uv_all = get_matching_lattices(
    ...
    if all_matches:
        #print ('Total {} matches found.'.format(len(uv_all)))
        # send each uv to functions below
        all_ifaces, n_subs, sd_indices = [], [], []
        for i in range(len(uv_all)):
            uv_substrate, uv_mat2d = uv_all[i][0] ,uv_all[i][1]
            try:
                iface, n_sub, sd_index = align_and_attach(
                                    uv_substrate, uv_mat2d, substrate_prim,
                                    twod_prim, nlayers_2d, nlayers_substrate,
                                    separation, sd_layers,
                                    max_angle_diff=max_angle_diff)
                all_ifaces.append(iface)
                n_subs.append(n_sub)
                sd_indices.append(sd_index)
            except:
                continue

        return all_ifaces, n_subs, sd_indices, uv_all
    else:
        return align_and_attach(uv_substrate, uv_mat2d, substrate_prim,
                                twod_prim, nlayers_2d, nlayers_substrate,
                                separation, sd_layers,
                                max_angle_diff=max_angle_diff)


def align_and_attach(uv_substrate, uv_mat2d, substrate_prim, twod_prim,
                     nlayers_2d, nlayers_substrate, separation, sd_layers,
                     max_angle_diff=1):
    """
    wrapper around get_aligned_lattices and get_interface

    Args:

    uv_substrate - list of lattice matched vectors of substrate
    uv_mat2d - list of lattice matched vectors of 2d material
    substrate_prim - structure of primitive substrate
    twod_prim - structure of 2D lattice
    max_angle_diff - maximum angle difference to match
    """
    n_prim_sub = substrate_prim.num_sites
    try:
        #get aligned lattices
        sub, mat2d = get_aligned_lattices(uv_substrate,   # matched lattice
                                          uv_mat2d,
                                          substrate_prim,
                                          twod_prim,
                                          max_angle_diff=max_angle_diff)
